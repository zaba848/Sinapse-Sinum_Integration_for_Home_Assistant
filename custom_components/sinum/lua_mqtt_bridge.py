"""Embedded Lua MQTT bridge template for the Sinum integration.

This module stores the canonical MQTT bridge script so it is always available
at runtime (HACS installations do not necessarily ship lua_scripts/).

``render(topic_prefix, client_id)`` returns the ready-to-upload Lua code.
"""
...
--
-- Managed by Home Assistant integration — do not edit manually.
-- Re-upload via Developer Tools > Services > sinum.upload_mqtt_bridge.
--
-- Supported device types:
-- v Virtual: thermostat, relay_integrator, blind_controller_integrator, gate, wicket,
--    dimmer_rgb_controller_integrator, dimmer_rgb_integrator, heat_pump_manager, custom_device
-- v WTP: temperature_sensor, humidity_sensor, temperature_regulator, fan_coil, fan_coil_v2,
...
--    iaq_sensor
-- v SBUS: fan_coil, temperature_sensor, humidity_sensor, two_state_input_sensor, button,
...
-- (optional if present: LoRa, SLINK, Video, Alarm system)
--
-- MQTT topics published:
--   <TOPIC_PREFIX>/state/<device_id>      Device state JSON
...
def render(topic_prefix: str, client_id: int) -> str:
    """Return the MQTT bridge Lua script with injected configuration."""
    return _TEMPLATE.format(topic_prefix=topic_prefix, client_id=client_id)
