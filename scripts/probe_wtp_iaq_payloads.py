...
async def probe_iaq_sensors(hub_url: str, api_token: str) -> dict:  # noqa: C901
    ...
    try:
        async with (
            ...
            session.get(f"{hub_url}/api/v1/devices", headers=headers) as resp,
        ):
            if resp.status != 200:
            ...
            for device in devices:
                if device.get("type") in iaq_types:
                    ...
                    for field in unexpected:
                        if field not in ["temperature", "humidity", "updated_at"]:
                            device_info["validation"]["issues"].append(
...
async def main():
    ...
    parser.add_argument(
        "--hub-url",
        required=True,
        help="Hub URL, for example http://sinum-wtp.local",
    )
